...
After at least one exchange, the user sends another Traditional Chinese message in the same page session and receives another streaming reply in the same single thread, with prior turns still visible.
...
**Independent Test**: Send two messages in sequence without leaving the page; confirm both pairs of turns remain visible and ordered.
...
- **Chat Thread**: The single conversation container visible in the UI for a page session; v1 allows exactly one thread per session.
